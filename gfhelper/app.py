...
class App(object):
    ...
    def search_script(self, path):
        ...
        for path in glob(os.path.join(path, '*.py')):
            ...
            self._logger.debug('Find script %s(%s)' % (name, doc))
            self._script[name] = (path, doc or '')

    def load_script(self, name, path):
        conf_path = os.path.splitext(path)[0] + '.yaml'
        try:
            imp.load_source(name, path)
            self._logger.debug('Load script: %s(%s)' % (name, path))
        except Exception:
            self._logger.error('Fail to load script: %s(%s)' % (name, path))

    def prepare(self):
        # init logger
        fileConfig(app.get_app_path('logging.ini'))
    ...
    def launch(self):
        ...
        self.load_script('test', self.get_app_path('gfhelper/script/test.py'))

        script_name = self.config.get('core.current_script')
        if script_name:
            try:
                self.load_script(script_name, self.scripts[script_name][0])
            except Exception:
                pass
